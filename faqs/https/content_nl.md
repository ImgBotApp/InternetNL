...
* **Vertrouwelijkheid**: De verbinding tussen de bezoeker en de website is versleuteld waardoor het onmogelijk is voor kwaadwillenden om de webcontent, URL's, cookies en andere gevoelige metadata onderweg af te luisteren.
* **Authenticiteit**: De bezoeker kan controleren dat hij/zij direct met de website die hoort bij de gebruikte domeinnaam contact heeft, en niet met een vervalste website of via een kwaadwillende tussenpersoon ('man-in-the-middle').
...
* Het gebruik van HTTPS biedt ook SEO-voordelen. SEO staat voor Search Engine Optimization en is een verzamelnaam voor een heleboel methoden en technieken die een website optimaal vindbaar en doorzoekbaar maken voor zoekmachines. Een betere vindbaarheid betekent immers meer (relevante) bezoekers. Bij Google is het bijvoorbeeld zo dat websites die worden aangeboden op basis van HTTPS, hoger in de ranglijst staan dan soortgelijke HTTP varianten.
* Eventuele discussies over het feit dat HTTPS websites (te) traag maakt, behoren definitief tot het verleden. HTTP/2 biedt vele voordelen ten opzicht van het huidige HTTP/1.1, waaronder een zeer snelle laadtijd en een verbeterde beveiliging. De snelheid van het inladen van een webpagina over HTTP/2 met TLS is, afhankelijk van de inhoud, vele malen sneller dan over HTTP/1.1 zonder TLS (zie [hier](https://www.httpvshttps.com/) en [hier](https://http2.akamai.com/demo)). Daarnaast valt op dat hoewel het vanuit de specificatie van HTTP/2 niet verplicht is, browserfabrikanten er massaal voor kiezen om HTTP/2 alleen te ondersteunen in combinatie met TLS. Dit is goed nieuws voor het gebruik van HTTPS dat op deze manier hopelijk de nieuwe standaard wordt.
...
Het gebruik van HTTPS is doorgaans te herkennen aan het slotje in de browser.
...
In theorie zorgt de toevoeging van TLS aan HTTP ervoor dat er meer data wordt verzonden tussen de client en de server. Daarnaast kan er ook wat tijd verloren gaan met het daadwerkelijk toepassen van de encryptie. In de praktijk is het verschil echter te verwaarlozen, aangezien computers tegenwoordig erg krachtig zijn en internetverbindingen voldoende snelheid bieden. In specifieke gevallen kan er gebruik worden gemaakt van specialistische apparatuur om de belasting van het toepassen van TLS te verplaatsen van een webserver naar een hiervoor geoptimaliseerd apparaat zoals een SSL offloader.
